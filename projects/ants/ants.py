...
class Insect:
    ...
    def reduce_health(self, amount):
        ...
        3
        """
        print("DEBUG:call Insect reduce_health")
        self.health -= amount
        if self.health <= 0:
            self.death_callback()
            print("DEBUG:done death_callback")
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        trans_time=0
        while now_place and not now_place.is_hive:
            if now_place.bees and self.min_range<=trans_time<=self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health-amount <= 0:
            ...
            total_damage=amount
        # damage every bee using reduce_health method for each bee
        # new_ls=[]
        # while 1:
        ...
        #     last_bee=bees_copy.pop()
        #     print("DEBUG:last_bee:",last_bee)
        #     print("DEBUG:last_bee type:",type(last_bee))
        #     last_bee.reduce_health(total_damage)
        #     print("DEBUG:done one reduce")
        #     if last_bee.health>0:
        #         new_ls.append(last_bee)
        #         print("DEBUG:append new bee:",last_bee,new_ls)
        # self.place.bees=new_ls
        # print("DEBUG:new list:",new_ls)
        new_ls=[]
        for index, bee in enumerate(self.place.bees[:]):
            bee.reduce_health(total_damage)
            print("DEBUG:bee result:",bee)
            if bee.health>0:
                new_ls.append(bee)
        self.place.bees=new_ls
        print("DEBUG:new list:",new_ls)
        ...
        # reduce amount from FireAnts health
        return super().reduce_health(amount)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        # check if its chewing
        if self.chew_timer:
            # if is, do nothing
            self.chew_timer-=1
            pass
            # if not, search for a bee to chew
        elif self.place.bees:
                # if there is a bee, chew it
                bee=random_bee(self.place.bees)
                bee.reduce_health(bee.health)
                # don't call bee.remove_from(self.place)
                # set chew timer
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while 1:
            if place_now is None:
                break

            if not place_now.ant:
                pass
            # check for contained ants
            elif place_now.ant.is_container and place_now.ant.ant_contained and place_now.ant.ant_contained not in gamestate.doubled_ls:
                place_now.ant.ant_contained.damage*=2
                gamestate.doubled_ls.append(place_now.ant.ant_contained)
            # for normal ants
            elif place_now.ant not in gamestate.doubled_ls:
                place_now.ant.damage*=2
                gamestate.doubled_ls.append(place_now.ant)
...
class GameState:
    ...
    def __init__(self, strategy, beehive, ant_types, create_places, dimensions, food=2):
        ...
        self.queen=False
        self.doubled_ls=[]   # all the doubled damage ants
